...
\[  L(1/\theta, \beta, \sigma^2_\gamma| \mathbf{y}, \mathbf{b})  =  \prod_{i=1}^{365} \left[ \int \left( \prod_{j=1}^{3} f(y_{ij}|\mu_{ij}) \right) f(b_i | \sigma^2_\gamma) \hspace{3 pt} db_i \right] \]
...
\begin{equation}
\begin{split} 
    l(1/\theta, \beta, \sigma^2_\gamma) = \sum_{i=1}^{365} log \bigg[ \int \prod_{j=1}^{3} \frac{\Gamma(y_{ij} + \theta)}{\Gamma(y_{ij} + 1)\Gamma(\theta)} 
...
We will need to sample the random effects $b_i$ for the $i-$th day. Let $Y_i=(y_{i1},y_{i2},y_{i3})^T$. The density function for $b_i$ given $Y_i, \beta, \theta, \sigma^2_\gamma$ is:
...
At the $r$th iteration of the MH algorithm with a current value $b_i^{(r)}$, a new $b_i$ is generated from $N(b^{(r)}, c(0)$ where $c(0)=(1+\sum_{j=1}^{3}\frac{\theta\cdot\mu_{ij}^0}{\mu_{ij}^0+\theta})^{-1}$ and $\mu_{ij}^0=\exp\{x_{ij}\beta\}$. The probability of accepting this new $b_i$ is:

\begin{equation}
    \min\bigg\{1, \frac{P(b_i | Y, \theta, \beta, \sigma^2_\gamma)}{P(b_i^{(r)} | Y, 1/\theta, \beta, \sigma^2_\gamma)}\bigg\}
\end{equation}.
