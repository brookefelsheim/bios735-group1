...
Thus, for the MCEM algorithm, we will be maximizing the expectation of the complete data log likelihood, otherwise known as the Q-function at step $t$. So the E-step is to calculate the Q-function which is defined as the following:
...
\end{split}
\end{equation}
>>>>>>> 1b531de8917318aaeb0a6441ed63da8ab012063e

We will use MCEM algorithm to solve the problem and use Metropolis Hastings Algorithm with a random walk to sample the random effects $b_i$ for the $i-$th day. Let $Y_i=(y_{i1},y_{i2},y_{i3})^T$. The density function for $b_i$ given $Y_i, \beta, \theta, \sigma^2_\gamma$ is:

\begin{equation}
\begin{split} 
    P(b_i | Y_i,\theta, \beta, \sigma^2_\gamma) \propto P(Y_i | b_i, \theta, \beta, \sigma^2_\gamma)\cdot P(b_i|\sigma^2_\gamma) \\ \propto \prod_{j=1}^{3} \bigg(\frac{\theta}{\mu_{ij}+\theta}\bigg)^{\theta}\cdot \bigg(\frac{\mu_{ij}}{\mu_{ij}+\theta}\bigg)^{y_{ij}}\cdot \exp\bigg(-\frac{b_i^2}{2\sigma^2_\gamma}\bigg) \\
    \propto \sum_{j=1}^{3} \bigg[ (y_{ij}\log\mu_{ij}-(y_{ij}+\theta)\log(\mu_{ij}+\theta))-\frac{b_i^2}{2\sigma^2_\gamma}\bigg]
\end{split}
\end{equation}

Here is our Metropolis Hastings Algorithm with a random walk: 
Step 1: Let $b_i^{*}$ be generated by  $b_i^{*}= b^{(t)}+ \epsilon$, where $\epsilon\sim U(-a,a)$. And we need to do parameter tuning on $a$ to to achieve around 20%-35% acceptance rate for optimal efficiency.
Step 2: Compute the Metropolis–Hastings ratio $R(b_i^{(t)},b_i^{*})$
 where $$ 
 R(b_i^{(t)},b_i^{*})=\frac{f(y_{ij}|\phi, b_i^{*})f(b^{*}|\sigma^2)}{f(y_{ij}|\phi, b_i^{(t)})f(b^{(t)}|\sigma^2)}
 $$
...
Also we will use Nelder-Mead in the M-step because it does not require 1st or 2nd derivatives and it is robust.


For the M-step, we will maximize the Q-function with respect to ${\theta, \beta, \sigma^2_\gamma}$. The Nelder-Mead method was used to optimize all of these parameters. After this, the E-step is repeated fro the enxt iteration, followed by the M-step again until the estimates for ${\theta, \beta, \sigma^2_\gamma}$ converge.
