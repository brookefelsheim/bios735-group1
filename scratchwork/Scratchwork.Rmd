...
## Random Forest
```{r}
x=seoul_data%>%select(Hour_chunks,Min_T,Max_T,Min_H,Max_H,Wind_speed,
              Rain_or_snow,Is_weekend,Season,Is_holiday,Date)%>%
  mutate(Hour_chunks=as.character(Hour_chunks,levels = c("[0,8)","[8,16)" ,"[16,24)")),
         Rain_or_snow=as.character(Rain_or_snow),
...
rf.fit$bestTune
rf.fit$results[rownames(rf.fit$bestTune),]$Rsquared
model1 <- randomForest(Bike_count ~ Hour_chunks + Min_T + Max_T + Min_H + Max_H + Wind_speed +
                         Rain_or_snow + Is_weekend + Season + Is_holiday + Date, 
                       data = data_all, importance=TRUE, ntree=500, mtry = 11, do.trace=100)

reprtree:::plot.getTree(model1,k=1,depth=4)
