...
ctree0=ctree(Bike_count ~ Hour_chunks + Min_T + Max_T + Min_H + Max_H + Wind_speed +
                         Rain_or_snow + Is_weekend + Season + Is_holiday + Date,  
             data = seoul_data_factor,control=ctree_control(maxdepth = 3))

plot(ctree0,gp = gpar(fontsize = 10,font=2),     # font size changed to 6
...
ctree0=ctree(Bike_count ~ Hour_chunks + Min_T + Max_T + Min_H + Max_H + Wind_speed +
                         Rain_or_snow + Is_weekend + Season + Is_holiday + Date,  
             data = seoul_data_factor)

ctree_mda=varimp(ctree0)
ctree_mda=sort(ctree_mda,decreasing = T)
...
model1 <- randomForest(Bike_count ~ Hour_chunks + Min_T + Max_T + Min_H + Max_H + Wind_speed +
                         Rain_or_snow + Is_weekend + Season + Is_holiday + Date, 
                       data = seoul_data, importance=TRUE, ntree=500, mtry = 6, do.trace=100)

reprtree:::plot.getTree(model1,k=10,depth=4,cex=1,font=2)
reprtree:::plot.getTree(model1,k=150,depth=4,cex=1,font=2)
...
y.pred_rf = predict(rf.fit, subset(seoul_data, select=-c(Bike_count)))
y.pred_ctree=predict(ctree0, subset(seoul_data_factor, select=-c(Bike_count)))
cor(y, y.pred_ctree, use = "pairwise.complete.obs")^2
cor(y, y.pred_rf, use = "pairwise.complete.obs")^2
...
set.seed(735)
model1 <- randomForest(Bike_count ~ Hour_chunks + Min_temp + Max_temp + Min_humidity + Max_humidity + Wind_speed+Rain_or_snow + Is_weekend + Season + Is_holiday+Date,data = train_factor, importance=TRUE, ntree=500,mtry = 7, do.trace=100)
...
y.pred_rf = predict(rf.fit, subset(test_factor, select=-c(Bike_count)))
y.pred_ctree=predict(ctree0, subset(test_factor, select=-c(Bike_count)))
cor(test_factor$Bike_count, y.pred_ctree, use = "pairwise.complete.obs")^2
cor(test_factor$Bike_count, y.pred_rf, use = "pairwise.complete.obs")^2
...
## Compare Fit of Models  - Random Forest
```{r}
...
model_fit_rf <- function(model, data, scale_seoul_mean = "no"){
...
model_fit_rf(rf.fit, seoul, scale_seoul_mean = "no") 
model_fit_rf(rf.fit, london_data, scale_seoul_mean = "yes") 
model_fit_rf(rf.fit, dc_data, scale_seoul_mean = "yes") 


```


## Compare Fit of Models  - GLMM
```{r}
load("test_full_model_10^-5_theta.Rdata")
model_glmm = model_test

glmm_model_fit <- function(model, data, scale_seoul_mean = "no"){
...
glmm_model_fit(model_glmm, seoul, scale_seoul_mean = "no") 
glmm_model_fit(model_glmm, london, scale_seoul_mean = "yes") 
glmm_model_fit(model_glmm, dc, scale_seoul_mean = "yes") 


```

```{r}
#z = read.csv("ranef.csv", header = FALSE)
#z[1,1] = -0.07438886
#z = as.matrix(z)
#z = t(z)
#z = as.numeric(z)
...
model_test = list(beta = c(7.519358384,  0.781888853,  1.038303216, -0.155615197, -0.265902287,
 -0.319954953, -0.198034786, -0.729883357, -0.014590067,  0.048136056,
 -0.005027722,  0.002630110,  0.015952248, -0.520325989),
 s2gamma = c(0.08611956),
 theta = 5.642231,
 eps = 7.737731e-06,
 qfunction = -9464.336,
 day_ranef = ranef,
...
save(model_test, file = "test_full_model_10^-5_theta.Rdata")
