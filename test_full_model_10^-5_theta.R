...
load("/nas/longleaf/home/qianqxj/Bike/seoul.rda")
...
fit_model = glm.nb(Bike_count ~ Hour_chunks + Is_weekend + Is_holiday + Season + Min_temp + 
                     Max_temp + Min_humidity + Max_humidity + Wind_speed + Rain_or_snow, data = seoul)
summary(fit_model)
...
# yi = seoul$Bike_count[seoul$Day == 1]
#X = model.matrix( Bike_count ~ Hour_chunks + Is_weekend + Is_holiday + Season + Min_temp + Max_temp + Min_humidity + Max_humidity + Wind_speed + Rain_or_snow, data = seoul)
#xi = X[seoul$Day == 1,]
