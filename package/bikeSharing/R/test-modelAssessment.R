...
test_that("errors for bad input to rf_model_fit", {
  expect_error(rf_model_fit(list(c(1, "test")), london, "yes", seoul))
  expect_error(rf_model_fit(glmm_fit, iris, "yes", seoul))
  expect_error(rf_model_fit(matrix(1:16, nrow = 4), london, "yes", seoul))
  expect_error(rf_model_fit(list(c(1, "test")), london, "yes", seoul))
  expect_error(rf_model_fit(london, "yes", seoul))
  expect_error(rf_model_fit())
})
